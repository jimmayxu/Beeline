...
class sergio(object):
    ...
    def build_graph (self, input_file_targets, input_file_regs, shared_coop_state = 0):
        ...
        # coop state, and coop_states in input_file_targets are ignored. Otherwise,
        # coop states are read from input file. Reasonbale values ~ 1-3
        # 2- input_file_targets: a csv file, one row per targets. Columns: Target Idx, #regulators,
        ...
        # 4- input_file_targets should not contain any line for master regulators
...
class SergioDataset(object):
    def __init__(
            self,
            number_genes: int = 400,
            number_sc: int = 2000,
            number_bins: int = 9,
            sergio_setting_targets: str = 'steady-state_input_GRN400G.txt',
            sergio_setting_regs: str = 'steady-state_input_MRs9bin400G.txt',
            save_path: str = 'SERGIO'
    ):
        self.number_genes = number_genes
        self.number_sc = number_sc
        self.number_bins = number_bins
        self.sergio_setting_targets = sergio_setting_targets
        self.sergio_setting_regs = sergio_setting_regs
        self.save_path = save_path
        self.input_file_targets = os.path.join(save_path, sergio_setting_targets)
        self.input_file_regs = os.path.join(save_path, sergio_setting_regs)


    def simulate(self, n=0):
        sim = sergio(number_genes=self.number_genes,
                     ...
                     sampling_state=15,
                     noise_type='dpd')
        sim.build_graph(input_file_targets=self.input_file_targets,
                        input_file_regs=self.input_file_regs, shared_coop_state=3)
        inPathData = '%s/expr_%ssc_%sbins%sG_%d.csv' % (self.save_path, self.number_sc, self.number_bins, self.number_genes, n)
        if os.path.isfile(inPathData):
            X_pd = pd.read_csv(inPathData)
            expr = np.zeros([self.number_bins, self.number_genes, self.number_sc])
            temp = X_pd.values.reshape(self.number_bins, self.number_sc, self.number_genes)
            for bin in range(self.number_bins):
                expr[bin, :, :] = temp[bin, :, :].T
            print('Clean version of dataset has been simulated at dir: %s' % inPathData)
        else:
            sim.simulate()
            expr = sim.getExpressions()
            expr_ = np.concatenate(expr, axis=1)

            X_pd = pd.DataFrame(expr_.T, columns=["Gene" + name for name in np.arange(self.number_genes).astype(str)])
            X_pd.to_csv(inPathData, index=False)
            print('Clean version of dataset is simulated and saved at %s' % inPathData)

        self.gene_names = X_pd.columns
        self.cell_names = X_pd.index
        self.expr = expr
        self.sim = sim
        X_ = sp_sparse.csc_matrix(X_pd.values)
        return X_

    def add_noise(self, noise=['Outlier', 'LibSize', 'Dropout', 'Poisson'], **params):
        assert all([i in ['Outlier', 'LibSize', 'Dropout', 'Poisson'] for i in noise])

        outlier_prob = params.get('outlier_prob', 0.3)
        libsize_mu = params.get('libsize_mu', 5.0)
        libsize_scale = params.get('libsize_scale', .1)
        dropout_percentile = params.get('dropout_percentile', 90.)
        expr = self.expr
        """Add outlier genes"""
        expr = self.sim.outlier_effect(expr, outlier_prob=outlier_prob, mean=2, scale=1) if 'Outlier' in noise else expr
        """Add Library Size Effect"""
        expr = self.sim.lib_size_effect(expr, mean=libsize_mu, scale=libsize_scale) if 'LibSize' in noise else expr
        """Add Dropouts"""
        binary_ind = self.sim.dropout_indicator(expr, shape=6.5, percentile=dropout_percentile) if 'Dropout' in noise else None
        expr = np.multiply(binary_ind, expr) if 'dropout' in noise else expr
        """Convert to UMI count"""
        expr = self.sim.convert_to_UMIcounts(expr) if 'Poisson' in noise else expr
        """Make a 2d gene expression matrix"""
        expr_ = np.concatenate(expr, axis=1)

        X_pd = pd.DataFrame(expr_.T, columns=self.gene_names)
        X_ = sp_sparse.csc_matrix(X_pd.values)
        return X_

    def adjacency_truth(self):
        groundtruth = np.loadtxt(self.input_file_targets, dtype=np.str)
        ground_temp = np.array([each.split(',')[:2] for each in groundtruth]).astype(np.float)
        number_child = ground_temp[:, 1].astype(int)
        children = ground_temp[:, 0].astype(int).tolist()
        parents = [each.split(',')[2:(2 + n)] for n, each in zip(number_child, groundtruth)]
        Adjacency_table_truth = np.zeros([self.number_genes, self.number_genes])
        true_edges = dict()
        for parent, child in zip(parents, children):
            parent = [int(float(i)) for i in parent]
            Adjacency_table_truth[np.ix_([child], parent)] = 1
            Adjacency_table_truth[np.ix_(parent, [child])] = 1
            parent = ['Gene%d' % i for i in parent]
            true_edges['Gene%d' % child] = parent
        return Adjacency_table_truth, true_edges

    def target_Genes(self):
        groundtruth = np.loadtxt(self.input_file_targets, dtype=np.str)
        ground_temp = np.array([each.split(',')[0] for each in groundtruth]).astype(np.float).astype(int)
        target_Genes = ["Gene" + str(g) for g in ground_temp]
